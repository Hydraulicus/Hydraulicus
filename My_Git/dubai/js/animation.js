function buttonInit(par) {
    ...
    function hideTracks(target) {
        var pathTag = Snap.selectAll("#" + target + " .animationstages");
...
function initAnimation(obj) {
    ...
    embdingSvgFileByAJAX(obj[0].svgWrap, obj[0].pathTosvgFile, function () {
        initCloseUp(obj[1]);
        if (obj[0].animated)  performOfWhollMap()
    });
};//end of init function

function initCloseUp(obj) {
    closeAnimTime = obj.AnimTime;
    embdingSvgFileByAJAX(obj.svgWrap, obj.pathTosvgFile, function () {
        if (obj.animated)  performOfCloseMap()
    });
}
...
function performOfCloseMap() {
    ...
    function shift_citygatetower() {
        citygatetower.stop().animate({"transform": "t0,-20 s1.25"}, 350, mina.backin,
            function () {
                citygatetower.animate({"transform": "t0,0 s1"}, 350, mina.backout)
            })
    }
}
...
function performOfWhollMap() {
    ...
    p1.initDraw();
    setTimeout(function () {
        Snap.selectAll(".animationstage1_5").forEach(function (element) {
    ...
    p1.callOnFinished = function () {
        p2.initDraw();
        Snap.selectAll(".animationstage1").forEach(function (element) {
            element.attr({"opacity": 1})
        });
        updateFig(2, {endvol: 21, x: 2146, y: 1937, color: "white"}, animTime[1]);
        setTimeout(function () {
            Snap.selectAll(".animationstage2_5").forEach(function (element) {
                element.attr({"opacity": 1})
            })
        }, animTime[1] * 0.9);
    };
    p2.callOnFinished = function () {
        p3.initDraw();
        Snap.selectAll(".animationstage2").forEach(function (element) {
            element.attr({"opacity": 1})
        });
        setTimeout(function () {
            Snap.selectAll(".animationstage3_5").forEach(function (element) {
                element.attr({"opacity": 1})
            })
        }, animTime[2] * 0.7);
    ...
    p3.callOnFinished = function () {
        Snap.selectAll(".animationstage3").forEach(function (element) {
            element.attr({"opacity": 1})
        });
        setTimeout(function () {
