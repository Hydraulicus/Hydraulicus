...
var segmentClick = function() {
    ...
    drawHexagonViewMode(targetId);
     ...
     Snap.select("#arrowright").attr({"cursor" : "pointer"}).click( function() { changeInfoIntoHexagonViewMode("pp") });
     Snap.select("#arrowleft").attr({"cursor" : "pointer"}).click( function() { changeInfoIntoHexagonViewMode("rs") } );
}
...
changeInfoIntoHexagonViewMode = function(par){
   console.log("changeInfoIntoHexagonViewMode",par); 
 group4clone.animate({"opacity" : 0}, rhythm, function(){cloneFlag = false; group4clone.addClass("visibility_hid"); putclone.clear(); collapseAllSegment (); drawHexagonViewMode (par) });
  Snap.selectAll(".icoInBar").forEach( function (element) { element.attr(passiveIconAttr); } ) ;
} 
